...
def generateSentence(models, desiredLength):
    ...
    while i <= desiredLength:
        ...
        while next_word == ('oooo' or 'oh' or 'oooooo'):
            next_word = selected_model.getNextToken(sentence)
        append(next_word)
        if i == 0:
            remove('^::^')
        ...
        if next_word == '$:::$':
            ...
            if '^:::^' in sentence:
                continue
            elif checkRhymableSentence(models, sentence):
                return sentence
            else:
                remove(sentence[-1])
...
def selectRhymingNGramModel(models, sentence):
    """
    Requires: models is a list of NGramModel objects sorted by descending
              priority: tri-, then bi-, then unigrams.
    Modifies: nothing
    ...
    # and returns the one that returns to true
    for i in range(len(models)):
        if models[i].trainingDataHasRhymingNGram(sentence):
            return models[i]
...
def generateRhymingSentence(models, rhyme_scheme, sentence1, desiredLength):
    ...
    for i in range(desiredLength):
        next_word = selected_model.getNextToken(sentence2)
        while next_word == ('oooo' or 'oh' or 'oooooo'):
            next_word = selected_model.getNextToken(sentence2)
        append(next_word)
        if i == 0:
...
def runRhymingLyricsGenerator(models):
    ...
    for i in range(len(append)):
        ...
        for j in range(4):
            if len(stanzas[i]) == 0:
                new_line = generateSentence(models, desiredLength)
                append[i](new_line)
                new_line = []
            elif rhyme_scheme[j] == rhyme_scheme[j - 1]:
                while len(stanzas[i]) < 4:
                    sentence1 = stanzas[i][line]
                    new_line = generateRhymingSentence(models, rhyme_scheme, sentence1, desiredLength)
                    append[i](new_line)
                    new_line = []
                    new_line = generateSentence(models, desiredLength)
                    append[i](new_line)
